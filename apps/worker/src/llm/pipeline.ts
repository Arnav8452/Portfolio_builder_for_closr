...
export async function executeWithRepair(
  ...
  CRITICAL UI FIELDS:
  1. 'achievements': Extract EXACTLY the top 1 to 10 MOST IMPRESSIVE achievements, major projects, or top content. Limit to a MAXIMUM of 10. Treat prominent repositories or projects as distinct achievements. You MUST extract a 'url' field if a link is present in the raw data. CRITICAL: NEVER list generic skills, languages, or tools (like "Docker", "React", or "Python") as achievements. Use the project/video name or metric as the 'title' with a brief summary as the 'description'. Return an array of these achievements.
