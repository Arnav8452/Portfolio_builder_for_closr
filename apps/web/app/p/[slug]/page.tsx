...
function GitHubCard({ metrics }: { metrics: any }) {
  ...
  const contribs = metrics?.contributions?.contributionsCollection;
  const nodes = metrics?.contributions?.repositories?.nodes || [];
  ...
  const topLanguages = Array.from(byLang.entries())
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5); // top 5
  ...
  return (
    <div className="glass-panel" style={{ position: "relative", overflow: "hidden" }}>
      ...
      <div className="stats-grid" style={{ marginBottom: "24px" }}>
        <div className="stat-card">
          <div className="stat-label">Commits (Yr)</div>
          <div className="stat-value">{contribs?.totalCommitContributions?.toLocaleString() || 0}</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Pull Requests</div>
          <div className="stat-value">{contribs?.totalPullRequestContributions?.toLocaleString() || 0}</div>
        </div>
        <div className="stat-card">
          <div className="stat-label">Issues Opened</div>
          <div className="stat-value">{contribs?.totalIssueContributions?.toLocaleString() || 0}</div>
        </div>
        <div className="stat-card">
      ...
      {topLanguages.length > 0 && (
        <div style={{ marginBottom: "24px" }}>
          <h3 style={{ color: "var(--arcade-purple)", marginBottom: "12px" }}>Top Languages By Bytes</h3>
          <div className="skills-tags">
            {topLanguages.map(([lang, bytes]) => (
              <span className="chip" key={lang}>
                <span style={{ color: "var(--arcade-ink)", fontWeight: 700, marginRight: "4px" }}>{lang}</span>
